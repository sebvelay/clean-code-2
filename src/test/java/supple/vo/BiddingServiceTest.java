...
public class BiddingServiceTest {
    ...
    @Test
    public void should_be_winning_when_no_other_bids() {
        whenThereAreNoBids();
        assertThat(outcomeOfPlacingMidBidInEuros()).isEqualTo(BidOutcome.WINNING);
    }

    @Test
    public void should_be_winning_when_bid_is_higher() {
        whenHighestBidIs(lowBid);
        assertThat(outcomeOfPlacingMidBidInEuros()).isEqualTo(BidOutcome.WINNING);
    }

    @Test
    public void should_be_losing_when_bid_is_lower() {
        whenHighestBidIs(highBid);
        assertThat(outcomeOfPlacingMidBidInEuros()).isEqualTo(BidOutcome.LOSING);
    }

    @Test
    public void should_be_winning_when_bid_has_same_amount_in_higher_currency() {
        whenHighestBidIs(midBidInDollars);
        assertThat(outcomeOfPlacingMidBidInEuros()).isEqualTo(BidOutcome.WINNING);
    }

    @Test
    public void should_be_losing_when_bid_has_same_amount_in_lower_currency() {
        whenHighestBidIs(midBidInEuros);
        assertThat(outcomeOfPlacingMidBidInDollars()).isEqualTo(BidOutcome.LOSING);
    }
